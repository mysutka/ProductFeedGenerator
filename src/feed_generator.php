...
abstract class FeedGenerator {
	...
	function exportTo($output_filename, $options=[]) {
		...
		if (php_sapi_name()=="cli" && isset($_SERVER["TERM"])) {
			print("generating {$format} feed {$output_filename}\n\n");
		}
		$this->logger->info("generating {$format} feed {$output_filename}");

		if ($this->_gen_feed($output_filename)) {
			$this->logger->info("{$format} product feed {$output_filename} successfully generated");
		} else {
			$this->logger->info("{$format} product feed {$output_filename} not generated");
		}
	}
	...
	protected function _gen_feed($output_filename) {
		...
		while (
			...
			$offset += $limit;
			if ($this->options["full_feed"] !== true) {
