...
The `-ff` flag forces a full feed generation (all items). This is useful in development where only the first batch is generated by default:
```bash
./scripts/robot_runner ProductFeedGenerator -ff
./scripts/robot_runner ProductFeedGenerator heureka_cz -ff
```
